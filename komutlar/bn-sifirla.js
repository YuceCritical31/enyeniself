...
message.reply(`${basari} ${message.author}, Başarıyla profil bannerini aşağıdaki görsel olarak kaydettim.`, new Discord.MessageAttachment(await db.get(`banner`)))
message.react('✅')
})
...
message.reply({content:`${basari} ${message.author}, Başarıyla profil bannerini aşağıdaki görsel olarak kaydettim.`, files:[await db.get(`banner`)]  })
message.react('✅')
}}catch{
...
if (args[0] === "sifirla") {
try{
if (!await db.get(`banner`)) return message.reply(`${basarisiz} ${message.author}, Profil fotoğrafı ayarlanmamış!`).then(x => setTimeout(() => {x.delete()}, 5000))
