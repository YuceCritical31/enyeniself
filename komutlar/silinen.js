...
if (!args[0]) return message.reply(`${basarisiz} ${message.author}, Dogru bi komut gir **${await db.get("prefix") || ayarlar.prefix}log <aç/kapat/remove/add>**.`).then(x => setTimeout(() => {x.delete()}, 5000));
if(!sayılar.some(word => message.content.includes(word))) return message.reply(`${basarisiz} ${message.author}, Dogru bi komut gir **${await db.get("prefix") || ayarlar.prefix}log <aç/kapat/remove>**`).then(x => setTimeout(() => {x.delete()}, 10000))
    
if (args[0] === "aç") {
if (await db.get("mesajlog") === "Aktif") return message.reply(`${basarisiz} ${message.author}, Log sistemi zaten açık.`).then(x => setTimeout(() => {x.delete()}, 5000));
  await db.set(`mesajlog`, "Aktif")
message.reply(`${basari} ${message.author}, Log sistemi açıldı.`).then(x => setTimeout(() => {x.delete()}, 5000));
}
    
if (args[0] === "kapat") {
if (!await db.get("mesajlog")) return message.reply(`${basarisiz} ${message.author}, Log sistemi zaten kapalı.`).then(x => setTimeout(() => {x.delete()}, 5000));
...
if (args[0] === "remove") {
  ...
  if(await db.get(`karaliste_${kullanici.id}`) === "Aktif") return message.reply(`${basarisiz} ${message.author}, Bu kullanıcıda zaten log tutmuyorum.`).then(x => setTimeout(() => {x.delete()}, 5000));
...
if (args[0] === "add") {
  ...
  if(!await db.get(`karaliste_${kullanici.id}`)) return message.reply(`${basarisiz} ${message.author}, Bu kullanıcıda zaten log tutuyorum.`).then(x => setTimeout(() => {x.delete()}, 5000));
  if(!kullanici) return message.reply(`${basarisiz} ${message.author}, Bir kullanıcı etiketlemelisin.`).then(x => setTimeout(() => {x.delete()}, 5000));
