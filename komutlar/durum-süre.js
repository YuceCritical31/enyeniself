...
if (!data){
message.reply(`${basari} ${message.author}, Durum zamanınız açıldı.`)
}
if (data) {
message.reply(`${basari} ${message.author}, Durum zamanınız açılıyor biraz bekleyin...`).then(msg => {
...
if (status === "kapat"){
if (!data2) return message.reply(`${basarisiz} ${message.author}, Durum zamanı zaten kapalı.`).then(x => setTimeout(() => {x.delete()}, 5000))
await db.delete(`durum_süresi`)
if (!data){
message.reply(`${basari} ${message.author}, Durum zamanınız kapatıldı.`)
}
if (data) {
message.reply(`${basari} ${message.author}, Durum zamanınız kapatılıyor biraz bekleyin...`).then(msg => {
    console.log(`BOT: Yeniden Başlatılıyor.....`);
    process.exit(0);
  })}
message.react('✅')
