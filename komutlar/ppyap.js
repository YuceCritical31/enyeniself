...
message.channel.send({content:`${basari} ${message.author}, Başarıyla profil fotoğrafını aşağıdaki görsel olarak yaptım.`, files:[x.url]})
})
...
message.channel.send({content:`${basari} ${message.author}, Başarıyla ${Atahan} adlı kullanıcının profil fotoğrafını kopyaladım.`, files:[Atahan.displayAvatarURL({dynamic: true, size: 1024})]})
}else if (message.attachments.size === 0 && args[0]) {
await client.user.setAvatar(args[0])
message.channel.send({content:`${basari} ${message.author}, Başarıyla profil fotoğrafını aşağıdaki görsel olarak yaptım.`, files:[args[0]]})
}}catch{
  message.channel.send(`${basarisiz} ${message.author}, Avatarını çok hızlı değişiyorsun veya yanlış kullanıcı/link/görsel giriyosun!`).then(x => setTimeout(() => {x.delete()}, 5000))
}
