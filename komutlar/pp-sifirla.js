...
message.reply(`${basari} ${message.author}, Başarıyla profil fotoğrafını aşağıdaki görsel olarak kaydettim.`, new Discord.MessageAttachment(await db.fetch(`avatar`)))
message.react('✅')
})
...
message.reply({content:`${basari} ${message.author}, Başarıyla profil fotoğrafını aşağıdaki görsel olarak kaydettim.`, files:[await db.fetch(`avatar`)]  })
message.react('✅')
}}catch{
...
if (args[0] === "sifirla") {
try{
if (!await db.fetch(`avatar`)) return message.reply(`${basarisiz} ${message.author}, Profil fotoğrafı ayarlanmamış!`).then(x => setTimeout(() => {x.delete()}, 5000))
await client.user.setAvatar(await db.fetch(`avatar`))
